...
איור: Notepad — טקסט פשוט בלי עזרה, מול VS Code — עורך צבעוני + טרמינל מובנה + איתור באגים, הכל בחלון אחד.
...
איור: שרשרת שמות קבצים מבולבלת (למעלה) מול שרשרת Commit-ים מסודרת עם מסלול חזרה (למטה, בטיל).
...
✔ **מה עושים בפועל:** בתרגול היום תיצרו Repository משלכם ב-GitHub — הבית הקבוע של הפרויקט לאורך הסמסטר.

איור: Git רץ על המחשב שלכם (היסטוריה מקומית); GitHub הוא העותק בענן, מגובה ומשותף — Push מעלה, Pull מוריד.
...
איור: בקשה בשפה טבעית → כלי AI → קוד שנכתב בפועל בקובץ שלכם.
...
איור: אותם 4 שלבי-פעולה: Autocomplete נוגע רק בכתיבת קוד; Agentic פועל בכל השלבים - קורא, כותב, בודק, ומתקן את עצמו.
...
✔ **מה עושים בפועל:** התשובה: 1+3 = Autocomplete מספיק (שינוי נקודתי); 2+4 = דורש Agentic (משתרע על כמה קבצים, דורש החלטות). תרחיש 4 הכי פחות חד-משמעי — זה בדיוק העניין.
...
איור: המחשה (לא לפי קנה-מידה): אופק הזמן גדל משמעותית בין דורות מודלים - מדקות בודדות לשעות ארוכות.
...
איור: 5 השלבים בזרימה מ-1 עד 5 (כזכור מהבולט למעלה - אם שלב 4 דורש תיקון, חוזרים לשלב 2).
...
✔ **מה עושים בפועל:** שוק הכלים משתנה מהר (בשיעור 1 ראינו את Gemini CLI החינמי נסגר תוך פחות משנה) — ולכן ההתקנה וההרשמה בפועל לכלי הנבחר קורות בתרגול המיידי, לא כאן. תמיד כדאי לבדוק סטטוס עדכני קרוב למועד השימוש.
